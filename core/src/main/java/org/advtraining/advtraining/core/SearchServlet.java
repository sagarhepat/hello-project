...
public class SearchServlet extends SlingSafeMethodsServlet {
	...
	public final void doGet(final SlingHttpServletRequest request,
		...
		try {
			// this is the current node that is requested, in case of a page
			// that is the jcr:content node
			Node currentNode = request.getResource().adaptTo(Node.class);
			PageManager pageManager = request.getResource()
					.getResourceResolver().adaptTo(PageManager.class);
	...
	private NodeIterator performSearch(Node queryRoot, String queryTerm)
		...
		// JQOM infrastructure
		QueryObjectModelFactory qf = queryRoot.getSession().getWorkspace()
		...
		Selector selector = qf
				.selector(SELECTOR_NT_UNSTRUCTURED, SELECTOR_NAME);

		// full text constraint
		Constraint constraint = qf.fullTextSearch(SELECTOR_NAME, null,
				qf.literal(vf.createValue(queryTerm)));
		// path constraint
		constraint = qf.and(constraint,
				qf.descendantNode(SELECTOR_NAME, queryRoot.getPath()));

		// execute the query without explicit order and columns
		QueryObjectModel query = qf.createQuery(selector, constraint, null,
				null);
